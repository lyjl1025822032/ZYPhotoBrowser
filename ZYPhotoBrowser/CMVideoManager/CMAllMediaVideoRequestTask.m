...
@interface CMAllMediaVideoRequestTask ()<NSURLConnectionDataDelegate, AVAssetResourceLoaderDelegate>
...
@implementation CMAllMediaVideoRequestTask
- (instancetype)init {
    self = [super init];
    if (self) {
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection {
    if (self.taskArr.count < 2) {
        _isFinishLoad = YES;
        //将请求url地址作为缓存本地文件的文件名
        NSString *videoFile = [NSString stringWithFormat:@"%@", _url.absoluteString.lastPathComponent];
        
        //这里自己写需要保存数据的路径
        NSString *document = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES).lastObject;
        NSString *movePath =  [document stringByAppendingPathComponent:videoFile];
